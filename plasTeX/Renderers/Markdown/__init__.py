...
class MarkdownRenderer(BaseRenderer):
    ...
    def do_cite(self, node):
        ...
        for bibkey in node.attributes["bibkeys"]:
                output.append("@" + bibkey)
    ...
    def do_textbf(self, node):
        return u'**%s**' % self.default(node)
    do_textmd = do_bf = do_textbf

    def do_textit(self, node):
        return u'*%s*' % self.default(node)
    do_textsl = do_it = do_sl = do_textit

    def do_texttt(self, node):
        return u'`%s`' % self.default(node)
    do_tt = do_texttt

    do_mdseries = do_bfseries = default
    do_rmfamily = do_textrm = do_sffamily = do_textsf = default
    do_ttfamily = do_upshape = do_textup = default
    do_itshape = do_scshape = do_textsc = default
    do_slshape = do_textnormal = do_rm = default
    do_cal = do_sc = default
    ...
    def do_math(self, node):
        return re.sub(r"\s*(?:\\\(|\$)\s*(.*?)\s*(?:\\\)|\$)\s*", r"$\1$", node.source).replace("\n", " ")
    ...
    def do_eqnarray(self, node):
        def render(node):
            return re.sub(r"\s*(?:\\\(|\\\[|\$\$?)?\s*(.*?)\s*(?:\\\)|\\\]|\$\$?)?\s*", r"$$\1$$", node.source).replace("\n", " ")
        s = map(render, node) #self.do_array(node, cellspacing=(0,0), render=render)
        output = []
        for line in s.split('\n'):
    ...
    def do_par(self, node):
        ...
        if numchildren == 1 and not isinstance(node[0], str):
            return u'%s\n\n' % str(node)
        elif numchildren == 2 and isinstance(node[1], str) and \
           not node[1].strip():
            return u'%s\n\n' % str(node)
        s = str(node)
        if not s: return u''
    ...
    def do_subsubsection(self, node):
        return u'\n\n\n#### %s' % (u'%s\n\n%s' % (node.fullTitle, node)).strip()
    do_paragraph = do_subparagraph = do_subsubparagraph = do_subsubsection
    ...
    def do_tikzpicture(self, node):
        return "```\n\\begin{tikzpicture}%s\\end{tikzpicture}\n```\n" % (re.sub(r'^\s*\n', r'', str(node)).rstrip())
    ...
    def do_frame(self, node):
        s = self.default(node) 
        return u'# %s' % s if len(s) > 0 else u""

    def do_frametitle(self, node):
        return u'# %s' % self.default(node)

    def do_includegraphics(self, node):
        if node.renderer.vectorImager.enabled:
                img = node.vectorImage.url
        else:
                img = node.image.url
        return u'![](%s){%s}' % (img, node.style.inline if node.style else "")
